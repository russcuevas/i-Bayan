...
// session
session_start();
require '../../vendor/autoload.php'; // or your correct path to PHPMailer
...
if (isset($_POST['accept_all'])) {
    ...
    echo "<script>alert('All family members and the account owner have been approved.'); window.location.href=window.location.href;</script>";
    exit();
}
...
if (isset($_POST['reject_all']) && !empty($_POST['rejection_note'])) {
    ...
    try {
        ...
        echo "<script>alert('Resident and family members deleted. Email sent.'); window.location.href='index.php';</script>";
        exit();
    } catch (Exception $e) {
                                    ...
                                    <div class="modal-dialog modal-lg" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <p><strong>Full Name:</strong> <?= htmlspecialchars($member['first_name'] . ' ' . $member['middle_name'] . ' ' . $member['last_name'] . ' ' . $member['suffix']) ?></p>
                                                <p style="text-transform: capitalize;"><strong>Relationship:</strong> <?= htmlspecialchars($member['relationship']) ?></p>
                                                <p><strong>Gender:</strong> <?= htmlspecialchars($member['gender']) ?></p>
                                                <p><strong>Birthdate:</strong> <?= htmlspecialchars($member['date_of_birth']) ?></p>
                                                <p><strong>Age:</strong> <?= htmlspecialchars($member['age']) ?></p>
                                                <p><strong>Civil Status:</strong> <?= htmlspecialchars($member['civil_status']) ?></p>
                                                <p><strong>Is Working:</strong> <?= $member['is_working'] ? 'Yes' : 'No' ?></p>
                                                <p><strong>Barangay Voted:</strong> <?= $member['is_barangay_voted'] ? 'Yes' : 'No' ?></p>
                                                <p><strong>Years in Barangay:</strong> <?= htmlspecialchars($member['years_in_barangay']) ?></p>
                                                <p><strong>Phone Number:</strong> <?= htmlspecialchars($member['phone_number']) ?></p>
                                                <p><strong>PhilHealth #:</strong> <?= htmlspecialchars($member['philhealth_number']) ?></p>
                                                <p><strong>School:</strong> <?= htmlspecialchars($member['school']) ?></p>
                                                <p><strong>Occupation:</strong> <?= htmlspecialchars($member['occupation']) ?></p>
                                                <p><strong>Status:</strong>
                                                    <span class="badge <?= $member['is_approved'] == 1 ? 'bg-green' : 'bg-orange' ?>">
                                                        <?= $member['is_approved'] == 1 ? 'Verified' : 'Pending' ?>
                                                    </span>
