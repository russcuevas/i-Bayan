...
$cert_stmt = $conn->prepare("SELECT * FROM tbl_closure WHERE resident_id = ? ORDER BY id DESC");
$cert_stmt->execute([$resident_id]);
$closure = $cert_stmt->fetchAll(PDO::FETCH_ASSOC);
                        ...
                        <div class="body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    ...
                                    <tbody>
                                        <?php foreach ($closure as $closures): ?>
                                            <tr>
                                                <td><?= htmlspecialchars($closures['owner_name']) ?></td>
                                                <td><?= htmlspecialchars($closures['certificate_type']) ?></td>
                                                <td>₱<?= number_format($closures['total_amount'], 2) ?></td>
                                                <td><?= htmlspecialchars($closures['purpose']) ?></td>
                                                <td><?= htmlspecialchars($closures['email']) ?></td>
                                                <td><?= htmlspecialchars($closures['contact']) ?></td>
                                                <td><?= date('F d, Y', strtotime($closures['created_at'] ?? 'now')) ?></td>
                                                <td>
                                                    <?php
                                                    $status = ucfirst($closures['status'] ?? 'Pending');
                                                ...
                                                <td>
                                                    <a href="closure_view_information.php?id=<?= $closures['id'] ?>" class="btn bg-teal waves-effect" style="margin-bottom: 5px;">
